...
function main() {
  const channels = [new EmailChannel(), new SMSChannel(), new PushNotificationChannel(), new PushNotificationChannel(), new PushNotificationChannel(), new SMSChannel(), new EmailChannel()];

  const alert = new AlertNotification(channels);

  alert.notify("Alerta de seguridad: Alguien enfrente de la casa");
